...
public class CgRkdSingleController extends BaseController
{
    ...
    @DataScope(deptAlias = "d", userAlias = "u")
    public TableDataInfo orderList(CgRkdSingleChild cgRkd)
    {
        startPage();
        List<CgRkdSingleChild> list = cgRkdChildService.selectCgRkdSingleOrderList(cgRkd);
        return getDataTable(list);
    }
            ...
            if(list.size()>0){
                for(List<String> info:list){
                    if(!"".equals(info.get(3))&&info.get(3)!=null) {
                        ...
                        child.setGoodsGg(info.get(5));//商品产地
    ...
    @Log(title = "进货单", businessType = BusinessType.UPDATE)
    @PutMapping("/editFp")
    public AjaxResult editFp(@RequestBody CgRkdSingle cgRkd)
    {
        CgRkdSingle item=cgRkdService.selectCgRkdSingleById(cgRkd.getId());
        if(item.getStatus()!=2){
            return  toAjaxByError("该订单未生效,禁止开票!");
        }
        //检查是否为已生效的订单
     ...
     * 取消订单生效
     */
    @PreAuthorize("@ss.hasPermi('system:cgrkdSingle:cancel')")
    @Log(title = "进货单取消生效", businessType = BusinessType.UPDATE)
    ...
    public AjaxResult cancel(@PathVariable String[] ids)
    {
        ...
        for(int i=0;i<ids.length;i++){
            ...
            //检测是否存在下级采购订单
            int num=cgRkdService.checkChildOrder(info.getDjNumber());
            if(num>0){
                return toAjaxByError(info.getDjNumber()+"：该单据存在下级采购订单!");
            }
        }
        return toAjax(cgRkdService.cancleCgRkdSingleStatus(ids));
    }
